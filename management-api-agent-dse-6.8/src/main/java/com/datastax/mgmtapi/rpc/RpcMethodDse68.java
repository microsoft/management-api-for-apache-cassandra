...
public class RpcMethodDse68 implements RpcMethod {
  private static final Logger logger = LoggerFactory.getLogger(RpcMethodDse68.class);
  ...
  private final ObjectSerializerDse68 retSerializer;
  private final OptionalInt clientStateArgIdx;
  private final List<Pair<Integer, RpcParam>> params;

  <R> RpcMethodDse68(Method method, RpcObject rpcObject) {
    this.method = method;
    ...
    this.argSerializers =
        params.stream()
            .map(p -> GenericSerializerDse68.getSerializer(genericParamTypes[p.getKey()]))
            .collect(Collectors.toList());

    this.argTypes =
        params.stream()
            .map(p -> GenericSerializerDse68.getTypeOrException(genericParamTypes[p.getKey()]))
            .collect(Collectors.toList());
    ...
    if (method.getAnnotation(Rpc.class).multiRow()) {
      ...
      this.retSerializer = new ObjectSerializerDse68<>((Class<?>) elemType);
    } else {
      this.retSerializer =
          new ObjectSerializerDse68<>(method.getReturnType(), method.getGenericReturnType());
    }
  }
