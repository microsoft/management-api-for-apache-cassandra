...
public class ObjectSerializerHcd<T> implements ObjectSerializer<T> {
   ...
   */
  public ObjectSerializerHcd(Class<T> clazz, Type genericType) {
    serializers =
        GenericSerializerHcd.simpleType(genericType)
            ? ImmutableSortedMap.<String, FieldSerializer>of(
                "result", new FieldSerializer(GenericSerializerHcd.getType(genericType), x -> x))
                ...
                Arrays.stream(clazz.getFields())
                    .collect(
                        Collectors.toMap(
                            ...
                            field ->
                                new FieldSerializer(
                                    GenericSerializerHcd.getType(field.getGenericType()), field))));
    // currently not recursive; multiple ways to do it
  }

  public ObjectSerializerHcd(Class<T> clazz) {
    this(clazz, clazz);
  }
