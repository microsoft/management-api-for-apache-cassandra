...
public class UnixSocketServerDse69 {
  private static final Logger logger = LoggerFactory.getLogger(UnixSocketServerDse69.class);
  ...
  public static ChannelInitializer<Channel> makeSocketInitializer(
    ...
    final Connection.Factory connectionFactory =
        (channel, version) -> new UnixSocketConnection(channel, version, connectionTracker, true);
  ...
  static class UnixSocketMessage extends SimpleChannelInboundHandler<Message.Request> {
    ...
    public void channelRead0(ChannelHandlerContext ctx, Message.Request request) {
      ...
      try {
            ...
            response -> {
              try {
                ...
                Message.Response resp =
                    response instanceof AuthenticateMessage ? new ReadyMessage() : response;
        ...
        if (t instanceof NullPointerException && t.getCause() != null) t = t.getCause();
    ...
    private void handleError(ChannelHandlerContext ctx, Message.Request request, Throwable error) {
      try {
        ...
        Message.UnexpectedChannelExceptionHandler handler =
            new Message.UnexpectedChannelExceptionHandler(ctx.channel(), true);
        ctx.writeAndFlush(
        ...
        // if the request connection is a server connection, we know that the assertion at the top
        // of the try block in channelRead0
  ...
  static class UnixSocketConnection extends ServerConnection {
    ...
    public UnixSocketConnection(
        Channel channel, ProtocolVersion version, Tracker tracker, boolean isUnixSocket) {
      ...
      switch (state) {
        ...
        case AUTHENTICATION:
          if (type != Message.Type.AUTH_RESPONSE)
            throw new ProtocolException(
                String.format("Unexpected message %s, expecting SASL_RESPONSE", type));
          break;
        case READY:
      ...
      switch (state) {
        case UNINITIALIZED:
          if (requestType == Message.Type.STARTUP) {
            if (responseType == Message.Type.AUTHENTICATE) state = State.AUTHENTICATION;
            else if (responseType == Message.Type.READY) state = State.READY;
          }
          break;
        case AUTHENTICATION:
