...
public class IPCController {
  ...
  private AbstractBootstrap createPipeline(
      EventLoopGroup eventLoopGroup,
      Map<ChannelOption, Object> channelOptions,
      ChannelInitializer<Channel> channelPipeline,
      boolean isClient) {
    ...
    if (channelOptions != null) {
      for (Map.Entry<ChannelOption, Object> option : channelOptions.entrySet()) {
        ...
        if (!isClient) b = ((ServerBootstrap) b).childOption(option.getKey(), option.getValue());
      }
    }
    ...
    return isClient
        ? b.handler(channelPipeline)
        : ((ServerBootstrap) b).childHandler(channelPipeline);
  }
  ...
  public static final class IPCBuilder {
    ...
    public IPCController build() {
      if (socketFile == null) throw new IllegalArgumentException("socketFile is required");

      if (eventLoopGroup == null) throw new IllegalArgumentException("eventLoopGroup is required");

      if (channelHandler == null) throw new IllegalArgumentException("channelHandler required");

      return new IPCController(
          eventLoopGroup, channelOptions, channelHandler, socketFile, isClient);
    }
  }
}
