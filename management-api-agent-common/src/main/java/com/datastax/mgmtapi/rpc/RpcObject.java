...
public class RpcObject {
  private static final RpcMethodServiceProvider RPC_METHOD_PROVIDER =
      new ServiceProviderLoader<RpcMethodServiceProvider>()
          .getProvider(RpcMethodServiceProvider.class);
  ...
  protected RpcObject(String name, Object object) {
    ...
    for (Method method : object.getClass().getMethods()) {
      if (method.isAnnotationPresent(Rpc.class)) {
        RpcMethod rpcMethod = RPC_METHOD_PROVIDER.getRpcMethod(method, this);
        if (found.containsKey(rpcMethod.getName())) {
          throw new AssertionError(
              String.format(
                  "Naming conflict in class %s: method %s already exists. "
