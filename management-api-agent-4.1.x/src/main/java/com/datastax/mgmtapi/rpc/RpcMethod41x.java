...
public class RpcMethod41x implements RpcMethod {
  private static final Logger logger = LoggerFactory.getLogger(RpcMethod41x.class);
  ...
  private final ObjectSerializer41x retSerializer;
  private final OptionalInt clientStateArgIdx;
  private final List<Pair<Integer, RpcParam>> params;

  <R> RpcMethod41x(Method method, RpcObject rpcObject) {
    this.method = method;
    ...
    this.argSerializers =
        params.stream()
            .map(p -> GenericSerializer41x.getSerializer(genericParamTypes[p.getKey()]))
            .collect(Collectors.toList());

    this.argTypes =
        params.stream()
            .map(p -> GenericSerializer41x.getTypeOrException(genericParamTypes[p.getKey()]))
            .collect(Collectors.toList());
    ...
    if (method.getAnnotation(Rpc.class).multiRow()) {
      ...
      this.retSerializer = new ObjectSerializer41x<>((Class<?>) elemType);
    } else {
      this.retSerializer =
          new ObjectSerializer41x<>(method.getReturnType(), method.getGenericReturnType());
    }
  }
