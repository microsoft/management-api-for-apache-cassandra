...
public class SocketUtils {
  ...
  /**
   * Helper method to create a valid Unix socket file path limited to {@link
   * #MAX_SOCKET_NAME}({@value #MAX_SOCKET_NAME}) characters.
   *
   * @param socketDir optional, directory to use instead of {@code java.io.tmpdir}. Can be {@code
   *     null}.
  ...
  public static String makeValidUnixSocketFile(String socketDir, String name) {
    ...
    if (socketDir == null) socketDir = System.getProperty("java.io.tmpdir");

    File fSocketDir = new File(socketDir);
    if (!fSocketDir.isDirectory() && !fSocketDir.mkdirs())
      throw new RuntimeException(
          String.format("%s is not a directory and could not be created", fSocketDir));

    try {

      /**
       * Tests create the socket files in a per-test-run directory. Withtout this hack, the socket
       * files would land in the per-Node instance directory (ng/SOME-UUID), which easily exceeds
       * the 103/107 character limit, so we put the socket files into the shorter built/test/tmp/xyz
       * directories.
       */
      ...
      if (f.getAbsolutePath().length() > SocketUtils.MAX_SOCKET_NAME) {
        logger.warn(
            "System property cassandra.collectd.socketdir overrides the default temp directory to '{}',"
                + "but its path name is too long. Falling back to /tmp",
            socketDir);
        socketDir = "/tmp";
    ...
    if (socketFile.length() > SocketUtils.MAX_SOCKET_NAME) {
      ...
      logger.warn(
          "The unix socket ({}) path is greater than the unix standard limit, cropping to {}",
          socketFile,
          tmp);
      socketFile = tmp;
    }
