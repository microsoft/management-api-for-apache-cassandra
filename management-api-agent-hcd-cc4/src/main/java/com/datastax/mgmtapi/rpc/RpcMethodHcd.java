...
public class RpcMethodHcd implements RpcMethod {
  private static final Logger logger = LoggerFactory.getLogger(RpcMethodHcd.class);
  ...
  private final ObjectSerializerHcd retSerializer;
  private final OptionalInt clientStateArgIdx;
  private final List<Pair<Integer, RpcParam>> params;

  <R> RpcMethodHcd(Method method, RpcObject rpcObject) {
    this.method = method;
    ...
    this.argSerializers =
        params.stream()
            .map(p -> GenericSerializerHcd.getSerializer(genericParamTypes[p.getKey()]))
            .collect(Collectors.toList());

    this.argTypes =
        params.stream()
            .map(p -> GenericSerializerHcd.getTypeOrException(genericParamTypes[p.getKey()]))
            .collect(Collectors.toList());
    ...
    if (method.getAnnotation(Rpc.class).multiRow()) {
      ...
      this.retSerializer = new ObjectSerializerHcd<>((Class<?>) elemType);
    } else {
      this.retSerializer =
          new ObjectSerializerHcd<>(method.getReturnType(), method.getGenericReturnType());
    }
  }
