...
public class RpcMethod4x implements RpcMethod {
  private static final Logger logger = LoggerFactory.getLogger(RpcMethod4x.class);
  ...
  private final ObjectSerializer4x retSerializer;
  private final OptionalInt clientStateArgIdx;
  private final List<Pair<Integer, RpcParam>> params;

  <R> RpcMethod4x(Method method, RpcObject rpcObject) {
    this.method = method;
    ...
    this.argSerializers =
        params.stream()
            .map(p -> GenericSerializer4x.getSerializer(genericParamTypes[p.getKey()]))
            .collect(Collectors.toList());

    this.argTypes =
        params.stream()
            .map(p -> GenericSerializer4x.getTypeOrException(genericParamTypes[p.getKey()]))
            .collect(Collectors.toList());
    ...
    if (method.getAnnotation(Rpc.class).multiRow()) {
      ...
      this.retSerializer = new ObjectSerializer4x<>((Class<?>) elemType);
    } else {
      this.retSerializer =
          new ObjectSerializer4x<>(method.getReturnType(), method.getGenericReturnType());
    }
  }
