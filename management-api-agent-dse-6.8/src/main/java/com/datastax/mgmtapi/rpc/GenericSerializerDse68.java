...
class GenericSerializerDse68 {
   ...
   *
   * <p>Extending this (at least for the purpose of RPC calls) is relatively straightforward: write
   * a class that extends C*'s TypeSerializer and add to the map. For actually getting data into
   * C*'s UDTs it might be a bit trickier. Unfortunately, there is not always a direct 1:1 mapping
   * between Java and Cassandra types. A simple example is millisecond timestamps, which could be
   * serialized as 'long' and 'timestamp'. The driver code actually has some bounds for this, but I
   * think for us it will be simpler to just write more serializers and add them to the map.
   */
  ...
  static void registerType(String className, AbstractType<?> type) {
    if (typeMap.putIfAbsent(className, type) != null) {
      throw new IllegalStateException("The type " + className + " is already registered.");
    }
  }

  static TypeSerializer getSerializer(Type type) {
    return getTypeOrException(type).getSerializer();
  }

  static AbstractType getTypeOrException(Type type) {
    AbstractType ctype = getType(type);

    if (ctype == null) {
      throw new AssertionError(
          String.format("Add type '%s' to GenericSerializer", type.getTypeName()));
    }

    return ctype;
  }

  static boolean simpleType(Type type) {
    return getType(type) != null;
  }

  /**
   * Most of the actual work is done here. Note that Generic type information is mostly destroyed at
   * runtime (a list is just a list). For the Parameterized types to work correctly you have to call
   * Method.getGenericParameterTypes() or something similar. Also, we currently punt on the frozen
   * keyword.
   *
   * @return The C* abstract type corresponding to the Java type, or null if not found/impossible.
   */
  static AbstractType getType(Type type) {
    assert type != null;
    String strType = type.getTypeName();

    /**
     * Rather than hard coding List<Integer> List<Long> List<String> etc we create them as needed.
     * Also there is no need for a lock as the actual serializers do that for us.
     */
    if (!typeMap.containsKey(strType)) {
      if (type instanceof ParameterizedType) {
        ...
        if (ptype.getRawType().getTypeName().equals("java.util.List")) {
          ...
          typeMap.putIfAbsent(
              strType, ListType.getInstance(getType(ptype.getActualTypeArguments()[0]), false));
        } else if (ptype.getRawType().getTypeName().equals("java.util.Set")) {
          assert ptype.getActualTypeArguments().length == 1;
          typeMap.putIfAbsent(
              strType, SetType.getInstance(getType(ptype.getActualTypeArguments()[0]), false));
        } else if (ptype.getRawType().getTypeName().equals("java.util.Map")) {
          assert ptype.getActualTypeArguments().length == 2;
          typeMap.putIfAbsent(
              strType,
              MapType.getInstance(
                  getType(ptype.getActualTypeArguments()[0]),
                  getType(ptype.getActualTypeArguments()[1]),
                  false));
        } else {
          throw new AssertionError(
              "Don't know how to serialize generic type '"
