...
public class ObjectSerializer41x<T> implements ObjectSerializer<T> {
   ...
   */
  public ObjectSerializer41x(Class<T> clazz, Type genericType) {
    serializers =
        GenericSerializer41x.simpleType(genericType)
            ? ImmutableSortedMap.<String, FieldSerializer>of(
                "result", new FieldSerializer(GenericSerializer41x.getType(genericType), x -> x))
  ...
  public ObjectSerializer41x(Class<T> clazz) {
    this(clazz, clazz);
  }
