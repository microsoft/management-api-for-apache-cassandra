...
public class CassandraRoleManagerInterceptor {
  private static final Boolean skipDefaultRoleSetup =
      Boolean.getBoolean("cassandra.skip_default_role_setup");
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(CassandraRoleManagerInterceptor.class);
  ...
  public static AgentBuilder.Transformer transformer() {
    return (builder, typeDescription, classLoader, javaModule, protectionDomain) ->
        builder
            .method(ElementMatchers.named("setupDefaultRole"))
            .intercept(MethodDelegation.to(CassandraRoleManagerInterceptor.class));
  }
        ...
        ShimLoader.instance
            ...
            .processQuery(
                String.format(
                    ...
                    AUTH_KEYSPACE_NAME, AuthKeyspace.ROLES, DEFAULT_SUPERUSER_NAME, ""),
  ...
  private static boolean hasExistingRoles() throws RequestExecutionException {
    ...
    String defaultSUQuery =
        String.format(
            "SELECT * FROM %s.%s WHERE role = '%s'",
            AUTH_KEYSPACE_NAME, AuthKeyspace.ROLES, "cassandra");
    String allUsersQuery =
        String.format("SELECT * FROM %s.%s LIMIT 1", AUTH_KEYSPACE_NAME, AuthKeyspace.ROLES);
    return !ShimLoader.instance.get().processQuery(defaultSUQuery, ConsistencyLevel.ONE).isEmpty()
        || !ShimLoader.instance
            .get()
            .processQuery(defaultSUQuery, ConsistencyLevel.QUORUM)
            .isEmpty()
        || !ShimLoader.instance
            .get()
            .processQuery(allUsersQuery, ConsistencyLevel.QUORUM)
            .isEmpty();
  }
}
