...
public class NettyJaxrsIPCServer extends NettyJaxrsServer {
  ...
  public NettyJaxrsServer setHttpChannelHandlers(final List<ChannelHandler> httpChannelHandlers) {
    this.httpChannelHandlers =
        httpChannelHandlers == null ? Collections.<ChannelHandler>emptyList() : httpChannelHandlers;
    return this;
  }
  ...
  public NettyJaxrsServer start() {
    synchronized (activeServerRef) {
      ...
      if (activeServer != null && activeServer.isActive()) return this;
  ...
  protected void setupHandlers(
      Channel ch, RequestDispatcher dispatcher, RestEasyHttpRequestDecoder.Protocol protocol) {
    ...
    channelPipeline.addLast(
        httpChannelHandlers.toArray(new ChannelHandler[httpChannelHandlers.size()]));
    channelPipeline.addLast(
        new RestEasyHttpRequestDecoder(dispatcher.getDispatcher(), root, protocol));
