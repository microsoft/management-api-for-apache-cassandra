...
public class ObjectSerializer4x<T> implements ObjectSerializer<T> {
   ...
   */
  public ObjectSerializer4x(Class<T> clazz, Type genericType) {
    serializers =
        GenericSerializer4x.simpleType(genericType)
            ? ImmutableSortedMap.<String, FieldSerializer>of(
                "result", new FieldSerializer(GenericSerializer4x.getType(genericType), x -> x))
  ...
  public ObjectSerializer4x(Class<T> clazz) {
    this(clazz, clazz);
  }
