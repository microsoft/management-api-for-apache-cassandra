...
public class RpcMethodDse69 implements RpcMethod {
  private static final Logger logger = LoggerFactory.getLogger(RpcMethodDse69.class);
  ...
  private final ObjectSerializerDse69 retSerializer;
  private final OptionalInt clientStateArgIdx;
  private final List<Pair<Integer, RpcParam>> params;

  <R> RpcMethodDse69(Method method, RpcObject rpcObject) {
    this.method = method;
    ...
    this.argSerializers =
        params.stream()
            .map(p -> GenericSerializerDse69.getSerializer(genericParamTypes[p.getKey()]))
            .collect(Collectors.toList());

    this.argTypes =
        params.stream()
            .map(p -> GenericSerializerDse69.getTypeOrException(genericParamTypes[p.getKey()]))
            .collect(Collectors.toList());
    ...
    if (method.getAnnotation(Rpc.class).multiRow()) {
      ...
      this.retSerializer = new ObjectSerializerDse69<>((Class<?>) elemType);
    } else {
      this.retSerializer =
          new ObjectSerializerDse69<>(method.getReturnType(), method.getGenericReturnType());
    }
  }
