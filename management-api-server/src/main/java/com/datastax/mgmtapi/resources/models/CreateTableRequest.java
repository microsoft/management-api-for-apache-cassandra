...
public class CreateTableRequest implements Serializable {
  ...
  public CreateTableRequest(
      @JsonProperty(value = "keyspace_name", required = true) String keyspaceName,
  ...
  private void validatePartitionKey() {
    Map<Integer, Integer> partitionKey =
        columns.stream()
            .filter(c -> c.kind == ColumnKind.PARTITION_KEY)
            .collect(
                Collectors.groupingBy(
                    c -> c.position, TreeMap::new, Collectors.summingInt(c -> 1)));
    ...
    for (int i = 0; i < partitionKey.size(); i++) {
      ...
      if (count == null) {
        ...
        throw new RuntimeException(
            String.format(
                "invalid primary key: found %d partition key columns at position %d", count, i));
      }
    }
  }

  private void validateClusteringColumns() {
    Map<Integer, Integer> clusteringColumns =
        columns.stream()
            .filter(c -> c.kind == ColumnKind.CLUSTERING_COLUMN)
            .collect(
                Collectors.groupingBy(
    ...
    for (int i = 0; i < clusteringColumns.size(); i++) {
      ...
      if (count == null) {
        throw new RuntimeException(
            "invalid primary key: missing clustering column at position " + i);
      } else if (count != 1) {
        throw new RuntimeException(
            String.format(
                "invalid primary key: found %d clustering columns at position %d", count, i));
      }
    }
  }
    ...
    for (Map.Entry<String, Object> entry : options.entrySet()) {
      ...
      if (value instanceof Map) {
        ...
        for (Map.Entry<?, ?> childEntry : map.entrySet()) {
          if (!(childEntry.getKey() instanceof String)
              || !(childEntry.getValue() instanceof String)) {
            throw new RuntimeException(
                String.format(
                    "invalid value for option '%s': expected String or Map<String,String>, got: %s",
                    key, value));
          }
        }
    ...
    public void validate(int index, CqlIdentifier keyspace) {
      ...
      if (StringUtils.isBlank(type)) {
        ...
        try {
          ...
          throw new RuntimeException(
              String.format("'columns[%d].type' is invalid: %s", index, e.getMessage()));
        }
      }
      if (kind == null) {
      ...
      if (kind == ColumnKind.PARTITION_KEY) {
        if (position < 0) {
          throw new RuntimeException(
              String.format(
                  "'columns[%d].position' must not be negative for partition key columns", index));
        }
      }
      if (kind == ColumnKind.CLUSTERING_COLUMN) {
        if (position < 0) {
          throw new RuntimeException(
              String.format(
                  "'columns[%d].position' must not be negative for clustering columns", index));
        }
        if (order == null) {
          throw new RuntimeException(
              String.format("'columns[%d].order' must not be empty for clustering columns", index));
        }
      }
    }
  }
